...
def conv2d_layer(inputs, weightDim, biasDim, strideDim, use_batch_norm, training_flag, layer_name):
	with tf.name_scope(layer_name) as scope:
		...
		if use_batch_norm is True:
			actv_map = tf.layers.batch_normalization(actv_map, axis=1, training=training_flag)
...
if __name__ == "__main__":
	...
	with tf.device('/device:GPU:0'):
		...
		actv_map1 = conv2d_layer(images, [5, 5, 3, 16], [16], [1, 1, 1, 1], BATCH_NORM, training_flag, 'CNN1')

		#Second convolutional layer
		actv_map2 = conv2d_layer(actv_map1, [5, 5, 16, 32], [32], [1, 2, 2, 1], BATCH_NORM, training_flag, 'CNN2')

		#Third convolutional layer
		actv_map3 = conv2d_layer(actv_map2, [4, 4, 32, 64], [64], [1, 2, 2, 1], BATCH_NORM, training_flag, 'CNN3')
		...
		with tf.name_scope('FC1') as scope:
			...
			if BATCH_NORM is True:
				fc1_output = tf.layers.batch_normalization(fc1_output, training=training_flag)

		#Apply dropout to reduce overfitting before readout layer (Not needed when using batch normalization)
		with tf.name_scope('dropout'):
			keep_prob = tf.placeholder(tf.float32, name='keep_prob') #probability that a neuron kept (not dropped)
		...
		with tf.name_scope('Readout') as scope:
			...
			predicted_labels = tf.add(tf.matmul(fc1_output, w_fc2), b_fc2, name='predicted_labels')
			predicted_results = tf.argmax(predicted_labels, axis=1, name='predicted_results')
		...
		with tf.name_scope('accuracy'):
			correct_results = tf.argmax(correct_labels, axis=1, name='correct_results')
			results_match = tf.equal(correct_results, predicted_results, name='results_match')
			accuracy = tf.reduce_mean(tf.cast(results_match, tf.float32), name='accuracy')
	...
	with tf.Session(config=config) as sess:
		...
		for i in range(EPOCHS):
			...
			for j in range(TRAIN_BATCHES_PER_EPOCH):
				...
				feed_dict = {images: image_data, correct_labels: labels, keep_prob: 0.5, training_flag: True}
			...
			for j in range(VALIDATION_BATCHES_PER_EPOCH):
				...
				feed_dict = {images: image_data, correct_labels: labels, keep_prob: 1.0, training_flag: False}
