...
import numpy as np
import time
import fifoQDataset as dataset
...
with tf.device('/device:GPU:0'):
	...
	images = tf.placeholder(tf.float32, shape=[None, dataset.image_size, dataset.image_size, 1], name='images')
...
with tf.Session(config=tf.ConfigProto(allow_soft_placement=True)) as sess:
	...
	if dataset.USING_FIFO_Q_DATASET is not None:
